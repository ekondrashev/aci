...
REPOS_JSON_PATH = os.environ.get('REPOS_JSON_PATH', os.path.join(os.path.dirname(__file__), 'repos.json'))
...
def index():
    if request.method == 'GET':
        ...
        if re.match(repo.get('branch'), branch, re.DOTALL) and repo.get('action', None):
            for platform in ci.Platforms(repo.get('platforms')):
                codes = []
                cwd = repo.get('cwd', '.')
                # if not os.path.existts(cwd):
                #     os.mkdir(cwd)
                with tempdir() as dirpath:

                    payload_path = os.path.join(dirpath, "payload.json")
                    with open(payload_path, "w+") as f:
                        f.write(json.dumps(payload, sort_keys=True, indent=4))
                    ...
                    with open(log_path, 'w') as log:
                        ...
                        for action in repo['action']:
                            formattings['retcode'] = code
                            cmdline = []
                            for cmd in action:
                                cmdline.append(cmd.format(**formattings))

                            code = platform.shell.execute(cmdline, err=log, out=log)
                            # subp = subprocess.Popen(cmdline, stderr=log, stdout=log)
                            # code = subp.wait()
                            codes.append(code)
